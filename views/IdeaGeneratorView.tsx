...
import { IdeaIcon, SendIcon } from '../components/Icons';
    ...
    return (
        <div className="flex flex-col h-full w-full p-4 md:p-8 overflow-y-auto">
            <header className="mb-8">
                <h1 className="text-3xl font-bold text-text-primary">AI Idea Generator</h1>
                <p className="text-text-secondary mt-1">Kickstart your creativity. Enter a topic and get inspired.</p>
            </header>

            <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="p-6 rounded-2xl glass-pane flex flex-col">
                    <label htmlFor="idea-topic" className="block text-lg font-semibold text-text-primary mb-4">
                        Your Topic
                    </label>
                    <textarea
                        id="idea-topic"
                        ...
                        placeholder="e.g., 'Marketing strategies for a new coffee shop' or 'Unique features for a mobile productivity app'"
                    ...
                    <button onClick={handleGenerate} disabled={isLoading} className="w-full flex items-center justify-center gap-2 mt-4 py-3 px-6 bg-brand-purple text-white font-semibold rounded-lg shadow-lg hover:bg-brand-purple/80 transition-all disabled:opacity-50 hover:shadow-glow-purple">
                        {isLoading ? 'Generating...' : 'Generate Ideas'} <SendIcon />
                    </button>
                </div>

                <div className="p-6 rounded-2xl glass-pane bg-bg-secondary/50 flex flex-col">
                    <div className="flex-shrink-0 flex items-center justify-between mb-4">
                        <h2 className="text-lg font-semibold text-text-primary">Generated Ideas</h2>
                    ...
                    <div className="flex-1 w-full p-4 bg-white rounded-lg border border-border-color whitespace-pre-wrap overflow-y-auto text-text-secondary">
                        {isLoading ? (
                            <div className="flex items-center justify-center h-full">
                                <IdeaIcon className="w-12 h-12 text-brand-purple animate-pulse" />
                            </div>
                        ) : result ? (
                            <div className="prose prose-sm max-w-none prose-p:text-slate-600 prose-headings:text-slate-800 prose-strong:text-slate-900 prose-li:text-slate-600">
                             ...
                             <div className="flex items-center justify-center h-full text-center">
                                <div>
                                    <IdeaIcon className="w-12 h-12 text-slate-400 mx-auto mb-2" />
