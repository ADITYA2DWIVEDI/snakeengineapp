...
export const SocialPlannerView: React.FC = () => {
    ...
    const handleGenerate = async () => {
        ...
        setResult('');
        try {
            const response = await generateSocialMediaPlan(topic);
            setResult(response);
        } catch (error) {
            console.error(error);
        ...
        <div className="flex flex-col h-full w-full p-4 md:p-8 overflow-y-auto">
            ...
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="p-6 rounded-2xl glass-pane flex flex-col">
                    <label htmlFor="social-topic" className="block text-lg font-semibold text-text-primary mb-4">
                        Topic or Brand
                    </label>
                    <textarea
                        id="social-topic"
                        ...
                        placeholder="e.g., 'A new eco-friendly coffee brand' or 'My personal blog about vintage photography'"
                    ...
                    <button onClick={handleGenerate} disabled={isLoading} className="w-full flex items-center justify-center gap-2 mt-4 py-3 px-6 bg-brand-purple text-white font-semibold rounded-lg shadow-lg hover:bg-brand-purple/80 transition-all disabled:opacity-50 hover:shadow-glow-purple">
                        {isLoading ? 'Generating...' : 'Generate Plan'} <SendIcon />
                    </button>
                </div>

                <div className="p-6 rounded-2xl glass-pane bg-bg-secondary/50 flex flex-col">
                    <div className="flex-shrink-0 flex items-center justify-between mb-4">
                        <h2 className="text-lg font-semibold text-text-primary">Generated Plan</h2>
                    ...
                    <div className="flex-1 w-full p-4 bg-white rounded-lg border border-border-color overflow-y-auto">
                        {isLoading ? (
                            <div className="flex items-center justify-center h-full">
                                <CalendarIcon className="w-12 h-12 text-brand-purple animate-pulse" />
                            </div>
                        ) : result ? (
