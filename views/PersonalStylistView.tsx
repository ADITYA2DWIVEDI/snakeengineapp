...
import { StyleIcon, SendIcon } from '../components/Icons';
...
export const PersonalStylistView: React.FC = () => {
    ...
    const handleGenerate = async () => {
        ...
        setResult('');
        try {
            const response = await getStyleAdvice(query);
            setResult(response);
        } catch (error) {
            console.error(error);
        ...
        <div className="flex flex-col h-full w-full p-4 md:p-8 overflow-y-auto">
            ...
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="p-6 rounded-2xl glass-pane flex flex-col">
                    <label htmlFor="style-query" className="block text-lg font-semibold text-text-primary mb-4">
                        Your Style Question
                    </label>
                    <textarea
                        id="style-query"
                        ...
                        placeholder="e.g., 'What should I wear for a job interview at a tech startup?' or 'Suggest an outfit for a casual summer date.'"
                ...
                <div className="p-6 rounded-2xl glass-pane bg-bg-secondary/50 flex flex-col">
                    <div className="flex-shrink-0 flex items-center justify-between mb-4">
                        <h2 className="text-lg font-semibold text-text-primary">Style Recommendations</h2>
                    ...
                    <div className="flex-1 w-full p-4 bg-white rounded-lg border border-border-color overflow-y-auto">
                        {isLoading ? (
                            <div className="flex items-center justify-center h-full">
                                <StyleIcon className="w-12 h-12 text-brand-purple animate-pulse" />
                            </div>
                        ) : result ? (
                            <div className="prose prose-sm max-w-none prose-p:text-slate-600 prose-headings:text-slate-800 prose-strong:text-slate-900 prose-li:text-slate-600">
                             ...
                             <div className="flex items-center justify-center h-full text-center">
                                <div>
                                    <StyleIcon className="w-12 h-12 text-slate-400 mx-auto mb-2" />
